...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
:- module(numlog, [learn/5,combination/1,maxClause/1,minClause/1,learnFromFile/2]).
...
combinations2([], []).
combinations2([_|Xs], Ys):-combinations2(Xs, Ys).
combinations2([X|Xs], [X|Ys]):-combinations2(Xs, Ys).
...
generateInterests(BKFile,ExampleFile,Rules,ArrPos,ArrNeg,Tot):-
    ...
    length(Rules,LRu),
    format('Number of interesting features:~w \nPlease wait...\nGenerating hypothesis space and looking for the best rule...',[LRu]).
...
learnFromFile(BKFile,ExampleFile):-
    ...
    minClause(MinClause),
    bagof(R => Acc, 
            (between(MinClause, MaxClause, N), combinations(N, Rules, Comb),partition(Comb,Comb1),getAccuracy(Comb1,ArrPos,ArrNeg,Tot,R,Acc)), 
            Result),
